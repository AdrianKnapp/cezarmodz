window.onload = () => {
  ...
  if (
    (urlWithoutGetParameters === 'http://localhost/CezarModz/index.php',
    'http://localhost/CezarModz/' ||
      presentUrl === 'http://localhost/CezarModz/')
  ) {
  ...
  const linkNextPageHref = document
    .querySelector('.pagination-input-next')
    .getAttribute('href');
  ...
  const linkPreviousPageHref = document
    .querySelector('.pagination-input-previous')
    .getAttribute('href');
  ...
  if (
    $('.pagination-button-right').hasClass(
      'pagination-button-right-disabled',
    ) &&
    $('.pagination-button-left').hasClass('pagination-button-left-disabled')
  ) {
...
function initFilterSystem() {
  ...
  if (filterCheckbox.length) {
    filterCheckbox.forEach((checkbox, index) =>
      checkbox.addEventListener('change', () =>
        blockOtherCheckboxes(checkbox.classList[1], index),
      ),
    );

    function blockOtherCheckboxes(inputClass, index) {
      const checkboxGroupToBlock = document.querySelectorAll('.' + inputClass);
      const checkboxHidden = document.getElementsByClassName(
        'js-' + inputClass,
      )[0];
...
function initDarkmodeSystem() {
  ...
  switchDarkMode.addEventListener('change', () => {
    if (switchDarkMode.checked) changeStyleToDark();
    else changeStyleToLight();
  });
}
initDarkmodeSystem();
...
function changeCssVariables(colors) {
  ...
  root.style.setProperty(
    '--color-border-to-darkmode',
    colors.colorBorderToDarkmode,
  );
  root.style.setProperty(
    '--color-pagination-button-disabled',
    colors.colorPaginationButtonDisabled,
  );
  root.style.setProperty(
    '--darkmode-toggle-system',
    colors.darkmodeToggleSystem,
  );
...
function initFilterButtonSystem() {
  ...
  filtrarButton.addEventListener('click', () => {
    checkboxes.forEach((checkbox, index) => {
      if (checkbox.checked) submitButton.click();
    });
  });
}
